...
# 特征重要性条形图
plt.figure(figsize=(10, 12))  # 增加高度到 12，确保 37 个标签清晰可见
# 不再使用 .head(20)，直接对所有筛选后的特征排序
all_feat_importances = pd.Series(
    rf_model.feature_importances_, 
    index=selected_feature_names
).sort_values(ascending=False)

# 使用条形图绘制
sns.barplot(x=all_feat_importances.values, y=all_feat_importances.index, palette='viridis')

plt.title('Feature Importance of All 37 Selected Predictors (Random Forest)')
plt.xlabel('Gini Importance')
plt.ylabel('Clinical Features')
plt.grid(axis='x', linestyle='--', alpha=0.7) # 添加虚线网格，方便对齐数值
plt.tight_layout() # 自动调整布局，防止左侧特征名称被遮挡
plt.savefig('3_all_feature_importance.png', dpi=300) # 提高分辨率
